...
def getData():
    ...
    def getDataAndTargets(targetMatrix):
        ...
        for i in range(len(targetMatrix)):
            data[i] = source_images[str(int(targetMatrix[i,0]))][:,:, None]
        return data, targets
    ...
    plt.imshow(x_test[0])
    plt.show()
    return x_train.astype('float32')/255., y_train.astype('float32'), x_test.astype('float32')/255.
...
x_train, validation_x, y_train, validation_y = train_test_split(x_train, y_train, test_size=0.2, random_state=41)
...
model.add(Conv2D(48, (3, 3), activation='relu', kernel_initializer='he_normal'))#, padding='same'))
...
sgd = keras.optimizers.SGD(lr = 0.005, momentum = 0.9, decay = 0.00002, nesterov=False)
